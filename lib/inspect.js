...
function inspectModule(zipPath) {
  ...
  const releaseTrack = String(manifest.releaseTrack || '').trim().toLowerCase();
  ...
  if (contractVersion >= 3 && !new Set(['stable', 'beta', 'nightly']).has(releaseTrack)) {
    add(errors, 'error', 'MANIFEST_RELEASE_TRACK', 'V3 releaseTrack must be stable, beta, or nightly.');
  }
...
function report(zipPath, manifest, errors, warnings, exports) {
  return {
    ...
    module: manifest ? {
      ...
      releaseTrack: manifest.releaseTrack || null,
    } : null,
    exports,
    errors,
    warnings,
